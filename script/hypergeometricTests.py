"""
This script is modified by yuxinghai at 20170918, two parts included,
one is figure size, the other is the output.
#################################################################
...
def enrichment_analysis(gene_list, tissue_df, alpha=0.05, aname='',
    ...
    ######################################
    ##########add functiopn to get enriched gene
    def get_enrichment_gene(x):
        gene_list=wanted_dic.index.values[wanted_dic[x]==1]
        gene_set=";".join(gene_list)
        return gene_set
    ...
    # add Gene set for every tissue
    df_final['Gene_Set'] = df_final.Tissue.apply(get_enrichment_gene)
    ################################################
